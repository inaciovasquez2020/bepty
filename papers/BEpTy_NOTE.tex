...
\section{URF-valued specialization}

For URF-facing graph applications, define
\[
...
\begin{theorem}[URF detection reformulation]
\(\mathrm{LocalExhaustion}_R(G)=0\) iff local \(R\)-ball cycles exhaust global cycle structure.
\end{theorem}

\begin{theorem}[High-girth target]
If \(\operatorname{girth}(G)>2R\), then \(Z_1(B_R(v))=0\) for every \(v\in V(G)\).
\end{theorem}

\begin{corollary}[High-girth reduction target]
If \(\operatorname{girth}(G)>2R\), then
\[
X_R(G)=Z_1(G).
\]
\end{corollary}
